...
with st.sidebar:
    ...
    if st.session_state.app_mode == "🎯 Callbreak":
        ...
        if st.session_state.pk_history:
            st.write("**🕒 Round History**")
            h_rows = [{"R": len(st.session_state.pk_history)-i, "Win": ", ".join(h["winners"]), "Pot": h["pot"]} for i, h in enumerate(reversed(st.session_state.pk_history))]
...
# =============================================================================
    ...
    with st.expander("🏆 Show Poker Hand Rankings (Highest to Lowest)"):
        ...
        # Try both .jpeg and .jpg just in case!
        img_path_jpeg = os.path.join(current_dir, "poker_order.png")
        img_path_jpg = os.path.join(current_dir, "poker_order.png")
        
        if os.path.exists(img_path_jpeg):
            # CHANGED: use_column_width instead of use_container_width
            st.image(img_path_jpeg, use_column_width=True) 
        elif os.path.exists(img_path_jpg):
            # CHANGED: use_column_width instead of use_container_width
            st.image(img_path_jpg, use_column_width=True)
        else:
    ...
    if not st.session_state.pk_buyin_confirmed:
        with st.form("pk_add", clear_on_submit=True):
            ...
            pb = c2.number_input("Buy-in", 5000, step=100)
            if st.form_submit_button("Add Player"):
                if pn:
    ...
    if st.session_state.pk_buyin_confirmed and pk_players:
        ...
        for i, p in enumerate(pk_players):
            ...
            st.markdown(f"<div class='card'> <span style='float:right; color:#888;'>{tag}</span><b>{p}</b>: ₹{bal} | <span class='{net_class}'>Net: {'+' if net > 0 else ''}{net}</span></div>", unsafe_allow_html=True)

        st.subheader("🎲 Current Round")
        if st.session_state.pk_reset_winners:
            if "pk_winners_widget" in st.session_state: st.session_state.pk_winners_widget = []
            st.session_state.pk_reset_winners = False

        sel_winners = st.multiselect("Winner(s)", pk_players, key="pk_winners_widget")
        
        conts, pot = {}, 0
        # Define the callback function BEFORE the loop
        ...
        for i, p in enumerate(pk_players):
            ...
            # The number input remains the same
            amt = c1.number_input(
                f"Bet: {p}", 
                0, 
            ...
            # Update the button to use the on_click callback instead of an if statement
            c2.button(
                "ALL-IN", 
                key=f"ai_{p}", 
                on_click=set_all_in, 
                args=(p,) # Pass the player name to the callback
            )
        ...
        if st.button("▶ Process Round", type="primary"):
            if not sel_winners: st.error("Select winners")
            else:
                for p in pk_players: st.session_state.pk_players[p] -= conts[p]
                for source in pk_players:
                    avail = conts[source]
                    eligible = [w for w in sel_winners if conts[w] > 0]
                    if eligible:
                        for w in eligible:
                            take = min(avail / len(eligible), conts[w])
                            st.session_state.pk_players[w] += take
                            avail -= take
                        if avail > 0: st.session_state.pk_players[source] += avail
                st.session_state.pk_dealer_idx = (st.session_state.pk_dealer_idx + 1) % len(pk_players)
                st.session_state.pk_history.append({"winners": sel_winners, "pot": pot})
                st.session_state.pk_reset_bets = True; st.session_state.pk_reset_winners = True
                st.rerun()
